export default function Fish({ direction = 'R', status = 'neutral', className = '' }) {
  ...
  // Rueda paper: Yellow fish. We use a bright yellow for the body.
  const bodyColor = '#FFD700'; 
  const strokeColor = '#1e293b'; // slate-800
  
  // Feedback colors if we want the fish to flash
  const feedbackColor = status === 'success' ? '#4ade80' : status === 'error' ? '#f87171' : bodyColor;

  return (
    <svg 
      viewBox="0 0 100 60" 
      className={`w-24 h-16 inline-block transition-transform duration-200 ${className} ${status === 'success' ? 'scale-110' : ''}`}
